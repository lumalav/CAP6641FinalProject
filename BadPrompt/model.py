...
class TransformerModelWrapper(object):
    ...
    def _save_config(self, path: str) -> None:
        with open(os.path.join(path, CONFIG_NAME), 'w', encoding='utf8') as f:
            f.write(jsonpickle.encode(self.config))

    @staticmethod
    def _load_config(path: str) -> WrapperConfig:
        with open(os.path.join(path, CONFIG_NAME), 'r', encoding='utf8') as f:
    ...
    def train(self,
        ...
        now = datetime.now()
        path_suffix = now.strftime('%m-%d_%H_%M_%S') + 'stage_%d' % stage
